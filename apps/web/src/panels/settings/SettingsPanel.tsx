...
export function SettingsPanel() {
  ...
  const [tab, setTab] = useState<TabKey>(() => {
    ...
    if (hint && ALL_TABS.includes(hint)) {
      delete (window as any).__tl_settingsTab;
  ...
  const settingsIcon = (t: string) => {
    ...
    switch (t) {
      case 'llm':
        return (
          <svg {...s} stroke={c}>
            <path d="M12 2l1.8 5.5 5.7.7-4.3 4 1.3 5.6-4.5-3.5-4.5 3.5 1.3-5.6-4.3-4 5.7-.7z" />
          </svg>
        );
      case 'apikey':
        return (
          <svg {...s} stroke={c}>
            ...
            <path d="M14 13l4 4M22 17l-2.5 2.5M19.5 19.5L16 16" />
          </svg>
        );
      case 'email':
        return (
      ...
      case 'git':
        return (
          <svg {...s} stroke={c}>
            ...
            <path d="M5 10c3 0 10-1 13-5" />
            <path d="M5 14c3 0 10 1 13 5" />
          </svg>
        );
      case 'standup':
        return (
      ...
      <div className="tabs-h">
        {ALL_TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={tab === t ? 'tab-h tab-h--active' : 'tab-h'}
            style={tab === t ? { color: tabColor[t], ['--tab-color' as any]: tabColor[t] } : undefined}
          >
            <span style={{ display: 'inline-flex', alignItems: 'center', opacity: tab === t ? 1 : 0.7 }}>
              {settingsIcon(t)}
            </span>
            {tabLabel(t)}
          </button>
