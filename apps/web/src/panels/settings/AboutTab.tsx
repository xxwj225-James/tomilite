...
function stored(key: string, fallback?: any) { try { const v = localStorage.getItem(key); return v ? JSON.parse(v) : (fallback ?? null); } catch { return fallback ?? null; } }
function store(key: string, val: any) { try { localStorage.setItem(key, JSON.stringify(val)); } catch {} }
...
export function AboutTab() {
  ...
  const resultText: Record<StatusKey, string> = {
    ...
    'downloaded': t('✅ 下载完成，点击顶部通知安装', '✅ ダウンロード完了 — 上部バナーからインストール', '✅ Downloaded — install from top banner'),
  ...
  const dlTimeout = (dlState?.lastProgress || dlState?.startTime) && (Date.now() - (dlState.lastProgress || dlState.startTime) > 1800000);
  ...
  const handleClick = () => {
    if (downloaded) {
      ...
      if (api?.checkUpdate) {
        api.checkUpdate();
        setTimeout(() => {
          setResultStatus(prev => prev === ('checking')
            ? ('up-to-date')
            : prev);
        }, 15000);
      } else {
  ...
  const btnLabel = dlError
    ...
    : <span style={{display:'inline-flex',alignItems:'center',gap:4}}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>{t('检查更新','更新をチェック','Check for Updates')}</span>;

  return (
    <div>
      <div className="card"><div className="card-hd">{t('关于','About','About')}</div><div className="card-bd text-sm">
        <p>TomiLite v{__APP_VERSION__}</p>
        {hasUpdate && <p className="text-xs text-brand-main mt-1">🚀 v{updateInfo.version} {t('可用','利用可能','available')}</p>}
        <p className="text-ink-muted mt-1">{t('AI 个人办公助手','AI パーソナルオフィスアシスタント','AI personal office assistant')}</p>
        <button className={(hasUpdate || downloaded) ? 'btn btn-brand btn-sm mt-2' : 'btn btn-sm mt-2'} style={(hasUpdate || downloaded) ? {} : { background: 'var(--surface2)', border: '1px solid var(--edge)' }} onClick={handleClick} disabled={(busy && !dlTimeout) || !!downloading}>
          {btnLabel}
        </button>
        ...
        {downloading && (
          <div style={{ marginTop: 8 }}>
            ...
            <div style={{ height: 4, background: 'var(--surface2)', borderRadius: 2, overflow: 'hidden' }}>
              <div style={{ width: `${Math.round(dlProgress)}%`, height: '100%', background: 'var(--brand)', borderRadius: 2, transition: 'width 0.3s ease' }} />
            </div>
          </div>
        )}
