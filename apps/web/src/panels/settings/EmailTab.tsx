...
const PRESETS: Record<
  string,
  {
    ...
    guide: { en: string[]; zh: string[]; ja: string[] };
  }
> = {
  QQ: {
    ...
    guide: {
      en: [
        'Open mail.qq.com → Settings → Account',
        'Enable "POP3/IMAP/SMTP service"',
      ...
      ja: [
        'mail.qq.com → 設定 → アカウントを開く',
        '「POP3/IMAP/SMTPサービス」を有効化',
        '「認証コードを生成」をクリック',
        'コードをコピーして貼り付け',
      ],
    },
  ...
  '163': {
    ...
    guide: {
      en: [
        'Open mail.163.com → Settings → POP3/SMTP/IMAP',
        'Enable "IMAP/SMTP service"',
        'Click "Add Authorization Code"',
        'Copy the code and paste here',
      ],
      zh: [
        '打开 mail.163.com → 设置 → POP3/SMTP/IMAP',
        '开启 "IMAP/SMTP服务"',
        '点击 "新增授权码"',
        '复制授权码粘贴到这里',
      ],
      ja: [
        'mail.163.com → 設定 → POP3/SMTP/IMAPを開く',
        '「IMAP/SMTPサービス」を有効化',
        '「認証コードを追加」をクリック',
        'コードをコピーして貼り付け',
      ],
  ...
  '126': {
    ...
    guide: {
      en: [
        'Open mail.126.com → Settings → POP3/SMTP/IMAP',
        'Enable "IMAP/SMTP service"',
        'Click "Add Authorization Code"',
        'Copy the code and paste here',
      ],
      zh: [
        '打开 mail.126.com → 设置 → POP3/SMTP/IMAP',
        '开启 "IMAP/SMTP服务"',
        '点击 "新增授权码"',
        '复制授权码粘贴到这里',
      ],
      ja: [
        'mail.126.com → 設定 → POP3/SMTP/IMAPを開く',
        '「IMAP/SMTPサービス」を有効化',
        '「認証コードを追加」をクリック',
        'コードをコピーして貼り付け',
      ],
  ...
  Gmail: {
    ...
    guide: {
      en: [
        ...
        'Copy the 16-char password and paste here',
      ],
      zh: [
        '打开 myaccount.google.com → 安全',
        '先开启 "两步验证"',
        '搜索 "应用专用密码"',
        '选择 "邮件" + "Windows 计算机" → 生成',
        '复制 16 位密码粘贴到这里',
      ],
      ja: [
        'myaccount.google.com → セキュリティを開く',
        'まず「2段階認証」を有効化',
        '「アプリパスワード」を検索',
        '「メール」+「Windows パソコン」→ 生成',
        '16文字のパスワードをコピーして貼り付け',
      ],
    },
  },
  Outlook: {
    ...
    guide: {
      ...
      ja: [
        'account.microsoft.com → セキュリティを開く',
        '「2段階認証」を有効化',
        '「アプリパスワード」へ',
        '新しいアプリパスワードを作成',
        'コピーして貼り付け',
...
// Provider brand colours, and the white envelope glyph drawn on top of them.
// Both are deliberately literal: Gmail's red is Gmail's red in either mode, and
// a white glyph on a saturated brand fill has to stay white — routing it
// through `--on-accent` would flip it to near-black under dark mode and erase
// the mark it is meant to draw.
    ...
    case 'Gmail':
      return (
        <svg {...s} viewBox="0 0 24 24">
          <rect x="2" y="4" width="20" height="16" rx="2" />
          <path d="M2 8l10 7 10-7" stroke="#fff" fill="none" />
          <path d="M6 18l4-5M18 18l-4-5" stroke="#fff" fill="none" />
        </svg>
      );
    case 'Outlook':
      return (
...
export function EmailTab() {
  ...
  const initialConfigRef = useRef<{ email: string; pass: string; imap: string; smtp: string; fromName: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    fetch('/api/email.getConfig', { signal: ac.signal })
      .then((r) => r.json())
      .then((d) => {
        ...
        try {
          ...
          if (imap?.config) {
            ...
            if (c.host) {
              setImapHost(c.host);
              loadedImap = c.host + ':' + (c.port || '993');
            }
            if (c.port) setImapPort(String(c.port));
            if (c.user) {
              setEmail(c.user);
              loadedEmail = c.user;
            }
            if (c.password) {
              setPass(c.password);
              loadedPass = c.password;
            }
            if (c.pollIntervalSeconds) setPollSeconds(String(c.pollIntervalSeconds));
          }
          if (smtp?.config) {
            const c = JSON.parse(smtp.config);
            if (c.host) {
              setSmtpHost(c.host);
              loadedSmtp = c.host + ':' + (c.port || '465');
            }
            if (c.port) setSmtpPort(String(c.port));
            if (c.user && !loadedEmail) {
              setEmail(c.user);
              loadedEmail = c.user;
            }
            if ((c.pass || c.password) && !loadedPass) {
              setPass(c.pass || c.password);
              loadedPass = c.pass || c.password;
            }
            if (c.fromName) {
              setFromName(c.fromName);
              loadedFrom = c.fromName;
            }
          }
          const curImap = loadedImap.split(':')[0];
          const curSmtp = loadedSmtp.split(':')[0];
          for (const [name, p] of Object.entries(PRESETS)) {
            if (p.imap.host === curImap && p.smtp.host === curSmtp) {
              setProvider(name);
              break;
            }
          }
      ...
      .catch(() => {});
    fetch('/api/email.imapStatus', { signal: ac.signal })
      .then((r) => r.json())
      .then((d) => {
        if (mountedRef.current) setConnected(Object.values(d.result?.data || {}).some((x: any) => x.connected));
      })
      .catch(() => {});
    fetch('/api/system.getConfig?input=' + encodeURIComponent(JSON.stringify({ key: 'imapLastError' })), {
      signal: ac.signal,
    })
      .then((r) => r.json())
      .then((d) => {
  ...
  const handleSaveAndConnect = async () => {
    ...
    if (smtpHost.trim()) {
      ...
      try {
        ...
        if (!td.result?.data?.ok) {
          setStatus(t('emailTab.smtpFailed', lang) + (td.result?.data?.error || t('emailTab.checkSettings', lang)));
          setSaving(false);
          return;
        }
      } catch {
        setStatus(t('emailTab.smtpConnFailed', lang));
        setSaving(false);
        return;
      }
    }
      ...
      if (imapHost.trim()) {
        await fetch('/api/email.saveIMAP', {
          ...
          body: JSON.stringify({
            ...
            tls: imapPort === '993',
            mailbox: 'INBOX',
            pollIntervalSeconds: parseInt(pollSeconds) || 60,
          }),
        });
      }
      if (smtpHost.trim()) {
        await fetch('/api/email.saveConfig', {
          ...
          body: JSON.stringify({
            ...
            config: JSON.stringify({
              ...
              pass: pass.trim(),
              starttls: smtpPort !== '465',
              fromName: fromName.trim() || 'TomiLite',
            }),
          }),
        });
      }
    } catch {
      setStatus(t('emailTab.saveFailed', lang));
      setSaving(false);
      return;
    }
    ...
    if (!connectingRef.current) {
      ...
      fetch('/api/email.connectIMAP', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
        ...
        .then((d) => {
          ...
          if (d.result?.data?.ok) {
            setConnected(true);
            setStatus(t('emailTab.connectedOk', lang));
          } else {
            setStatus(`❌ ${d.result?.data?.error || t('emailTab.imapConnectFailed', lang)}`);
          }
        })
        .catch((e) => {
          if (mountedRef.current) setStatus(`❌ ${e?.message || t('emailTab.networkError', lang)}`);
        })
        .finally(() => {
      ...
      <div className="card-bd">
        ...
        <div style={{ marginBottom: 12 }}>
          <p style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 6, fontWeight: 500 }}>
            {t('emailTab.chooseProvider', lang)}
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4 }}>
            {Object.keys(PRESETS).map((name) => (
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  border: provider === name ? '1.5px solid var(--brand)' : '1.5px solid var(--edge)',
                  background: provider === name ? 'var(--brand-soft)' : 'var(--surface2)',
              ...
              >
                ...
                >
                  <ProviderLogo name={name} size={16} />
                </span>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 'var(--radius-sm)',
                cursor: 'pointer',
                border: !provider ? '1.5px solid var(--brand)' : '1.5px solid var(--edge)',
                background: !provider ? 'var(--brand-soft)' : 'var(--surface2)',
                transition: 'all var(--transition-fast)',
                display: 'flex',
                alignItems: 'center',
                gap: 6,
        ...
        <div className="form-grp">
          ...
          <input
            ...
            placeholder={t('emailTab.appPasswordHint', lang)}
          />
          {/* ─── Auth code guide ─── */}
          {guide && (
            <div
            ...
            >
              <p style={{ fontWeight: 600, color: 'var(--amber)', marginBottom: 4 }}>
                🔑 {t('emailTab.howToGetAuthCode', lang).replace('{provider}', provider)}
              </p>
              <ol style={{ paddingLeft: 16, margin: 0 }}>
                {(guide[lang as keyof typeof guide] || guide.en).map((step, i) => (
                  <li key={i}>{step}</li>
                ))}
              </ol>
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
            <div className="form-grp" style={{ flex: 1 }}>
              ...
              <input
                className="form-input"
                style={{ fontSize: 11 }}
                value={imapHost ? `${imapHost}:${imapPort}` : ''}
                onChange={(e) => {
            ...
            <div className="form-grp" style={{ flex: '0 0 auto', minWidth: 70 }}>
              <label className="form-label" style={{ whiteSpace: 'nowrap' }}>
                {t('emailTab.pollSeconds', lang)}
              </label>
              <input
                className="form-input"
                style={{ fontSize: 11, textAlign: 'center', width: 60 }}
                value={pollSeconds}
            ...
            <div className="form-grp" style={{ flex: 1 }}>
              ...
              <input
                className="form-input"
                style={{ fontSize: 11 }}
                value={smtpHost ? `${smtpHost}:${smtpPort}` : ''}
                onChange={(e) => {
                  const [h, p] = e.target.value.split(':');
                  setSmtpHost(h || '');
                  setSmtpPort(p || '465');
        ...
        {status && (
          <p
            style={{
              fontSize: 11,
              margin: '8px 0 4px',
              color: status.includes('✅') ? 'var(--green)' : status.includes('❌') ? 'var(--red)' : 'var(--muted)',
            }}
          >
            {status}
          </p>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'flex-end', marginTop: 8 }}>
          <button
            ...
            onClick={connected ? handleDisconnect : handleSaveAndConnect}
            disabled={saving || !email.trim() || !pass.trim()}
          >
            {saving
              ? t('emailTab.saving', lang)
              : connected
                ? t('emailTab.disconnect', lang)
                : t('emailTab.saveAndConnect', lang)}
          </button>
        </div>
      </div>
    </div>
