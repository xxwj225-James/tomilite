    ...
    <div style={{ display: 'flex', height: '100%' }}>
      ...
      >
        <div style={{ textAlign: 'center', padding: '4px 0', borderBottom: '1px solid var(--edge)' }}>
          <div className="stat-val" style={{ color: 'var(--brand)', fontSize: 22 }}>
            {todo}
          </div>
          <div className="stat-lbl">Todo</div>
        </div>
        <div style={{ textAlign: 'center', padding: '4px 0', borderBottom: '1px solid var(--edge)' }}>
          <div className="stat-val" style={{ color: 'var(--amber)', fontSize: 22 }}>
            {inProgress}
          </div>
          <div className="stat-lbl">Active</div>
        </div>
        <div style={{ textAlign: 'center', padding: '4px 0', borderBottom: '1px solid var(--edge)' }}>
          <div className="stat-val" style={{ color: 'var(--green)', fontSize: 22 }}>
            {done}
          </div>
          <div className="stat-lbl">Done</div>
        </div>
        <div style={{ textAlign: 'center', padding: '4px 0' }}>
          <div className="stat-val" style={{ color: 'var(--blue)', fontSize: 22 }}>
            {rate}%
          </div>
          <div className="stat-lbl">Done%</div>
        </div>
...
export function HomePanel({ active = true }: { active?: boolean }) {
  ...
  const refreshMotto = async () => {
    ...
    try {
      const resp = await fetch('/api/system.generateMotto', {
        method: 'POST',
  ...
  useEffect(() => {
    ...
    fetch(`/api/system.getMotto?input=${encodeURIComponent(JSON.stringify({ lang }))}`)
      .then((r) => r.json())
      .then(async (d) => {
        if (d.result?.data) {
          setMotto(d.result.data);
          return;
        }
        const fallback = tr(
          lang,
          '每一天的代码，都在塑造未来的自己。',
          'すべてのコードがあなたの未来を形作る。',
          'Every line of code shapes your future.',
        );
        setMotto(fallback);
        try {
          const resp = await fetch('/api/system.generateMotto', {
  ...
  const dimLabels: Record<string, string> = {
    completion: t('home.completion', lang),
    velocity: t('home.velocity', lang),
    git_activity: t('home.git', lang),
    staleness: t('home.freshness', lang),
  };
  const dimColors: Record<string, string> = {
    completion: 'var(--green)',
    velocity: 'var(--blue)',
    git_activity: 'var(--amber)',
    staleness: 'var(--muted)',
  };
  ...
  return (
    <div
      className="p-2"
      style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'auto', gap: 10, paddingBottom: 80 }}
    >
      {motto && (
        <div
          onClick={refreshMotto}
          title={lang === 'zh' ? '点击刷新' : lang === 'ja' ? 'クリックで更新' : 'Click to refresh'}
          style={{
            padding: '12px 16px',
            ...
            textAlign: 'center',
            cursor: 'pointer',
            opacity: mottoRefreshing ? 0.6 : 1,
            transition: 'opacity 0.2s',
          }}
        >
          <div
            style={{ fontSize: 13, fontWeight: 500, lineHeight: 1.5, fontStyle: 'italic', color: 'var(--on-accent)' }}
          >
            {mottoRefreshing ? (lang === 'zh' ? '⏳ ...' : lang === 'ja' ? '⏳ ...' : '⏳ ...') : motto}
          </div>
        </div>
      )}
      ...
      {taskStats && (
        <div className="card" style={{ flexShrink: 0 }}>
          ...
          <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 9, color: 'var(--muted)', marginBottom: 4, fontWeight: 600 }}>
                {t('home.priority', lang)}
              </div>
              {(Object.entries(taskStats.byPriority || {}) as [string, number][]).map(([p, n]) => (
                <div key={p} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                  ...
                  <div
                    style={{ flex: 1, height: 14, background: 'var(--surface2)', borderRadius: 7, overflow: 'hidden' }}
                  >
                    <div
                      style={{
                        height: '100%',
                        width: (taskStats.total > 0 ? Math.round(((n as number) / taskStats.total) * 100) : 0) + '%',
                        background:
                          p === 'critical'
                            ? 'var(--brand)'
                            : p === 'high'
                  ...
                  <span style={{ fontSize: 10, color: 'var(--muted)', width: 18, textAlign: 'right' }}>
                    {n as number}
                  </span>
                </div>
              ))}
      ...
      <div className="card" style={{ flexShrink: 0 }}>
        <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--edge)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="btn-ghost btn-xs" onClick={() => fetchHealth(true)} disabled={healthLoading}>
              <svg
                width="12"
                height="12"
                viewBox="0 0 24 24"
                ...
                strokeLinecap="round"
                strokeLinejoin="round"
                style={healthLoading ? ({ animation: 'spin 1s linear infinite' } as any) : {}}
              >
                <polyline points="23 4 23 10 17 10" />
                <path d="M20.49 15a9 9 0 11-2.12-9.36L23 10" />
              </svg>
          ...
          {healthGeneratedAt && (
            <div style={{ fontSize: 9, color: 'var(--muted)', marginTop: 2 }}>{healthGeneratedAt}</div>
          )}
        </div>
        {health && (
          <div style={{ padding: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              <div
                style={{
                  ...
                  height: 56,
                  borderRadius: '50%',
                  background: `conic-gradient(var(--brand) ${health.score}%, var(--surface2) 0)`,
                  display: 'flex',
                  alignItems: 'center',
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--ink)', lineHeight: 1.5, marginTop: 2 }}>
                  {health.summary?.split('\n')[0]}
                </div>
              </div>
            </div>
            ...
            {health.dimensions &&
              Object.entries(health.dimensions).map(([key, val]: [string, any]) => (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span style={{ fontSize: 10, width: 60, color: 'var(--muted)' }}>{dimLabels[key] || key}</span>
                  <div
                    style={{ flex: 1, height: 6, background: 'var(--surface2)', borderRadius: 3, overflow: 'hidden' }}
                  >
                    <div
                      style={{
                        height: '100%',
                        width: `${val}%`,
                        background: dimColors[key] || 'var(--brand)',
                        borderRadius: 3,
              ...
              <div style={{ marginTop: 12, padding: '10px 12px', background: 'var(--surface2)', borderRadius: 8 }}>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          flex: 1,
                          height: `${Math.max(8, s)}%`,
                          background: s >= 60 ? 'var(--green)' : s >= 40 ? 'var(--amber)' : 'var(--brand)',
                ...
                <div style={{ fontSize: 11, color: 'var(--ink)', fontWeight: 500 }}>
                  {health.trend.direction === 'up'
                    ? t('health.trend.up', lang)
                    : health.trend.direction === 'down'
                      ? t('health.trend.down', lang)
                      : health.trend.steady
