  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <ConfirmDialog
        open={!!s.sendResult}
        variant="alert"
        title={s.sendResult?.key === 'success' ? _t('成功', '成功', 'Success') : _t('错误', 'エラー', 'Error')}
        message={s.sendResultMsg as string}
        lang={lang}
        onConfirm={() => s.setSendResult(null)}
        onCancel={() => s.setSendResult(null)}
      />
      <ConfirmDialog
        open={s.reportPendBack as boolean}
        title={_t('未保存的更改', '未保存の変更', 'Unsaved Changes')}
        message={_t('要保存后退出吗？', '保存してから退出しますか？', 'Save before leaving?')}
        lang={lang}
        confirmLabel={
        ...
        onConfirm={async () => {
          (s.setReportPendBack as (v: boolean) => void)(false);
          await (s.handleSave as () => Promise<void>)();
          (window as any).__tl_unsaved = null;
          (s.setSelected as (v: null) => void)(null);
          (s.setTitle as (v: string) => void)('');
          (s.setContent as (v: string) => void)('');
          (s.setCurrentReportId as (v: null) => void)(null);
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          if (r && (r as any) !== '__back__') {
            (s.setSelected as (v: Record<string, unknown>) => void)(r);
            (s.setTitle as (v: string) => void)(r.title as string);
            (s.setContent as (v: string) => void)((r.content as string) || '');
            (s.setReportType as (v: string) => void)((r.reportType as string) || 'daily');
