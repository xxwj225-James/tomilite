...
export function useReportsState(
  ...
  const sendResultText = (r: SendResult | null, lang: string): string => {
    ...
    const msgs: Record<string, Record<string, string>> = {
      ...
      'smtp-fail': {
        zh: `❌ ${r.errorDetail || '发送失败。检查 SMTP 配置。'}`,
        ja: `❌ ${r.errorDetail || '送信失敗。SMTP設定を確認してください。'}`,
        en: `❌ ${r.errorDetail || 'Send failed. Check SMTP config.'}`,
      },
      fail: {
        zh: '❌ 发送失败。请在 Settings → Email 中检查 SMTP 配置。',
        ja: '❌ 送信失敗。Settings → Email でSMTP設定を確認してください。',
        en: '❌ Send failed. Please check SMTP config in Settings → Email.',
      },
    };
  ...
  const fetchReports = async () => {
    try {
      const d = await api.report.list(50);
  ...
  useEffect(() => {
    ...
    window.addEventListener('tl-select-report', h);
    const onCloseEditor = () => {
      setSelected(null);
      setTitle('');
      setContent('');
      onEditingReport?.(null as any);
    };
    window.addEventListener('tl-close-report-editor', onCloseEditor);
    const consumePending = () => {
      const pending = (window as any).__tl_pendingReportSelect;
      if (pending) {
        (window as any).__tl_pendingReportSelect = null;
        h({ detail: pending } as any);
      }
    };
    consumePending();
    return () => {
      window.removeEventListener('tl-select-report', h);
      window.removeEventListener('tl-close-report-editor', onCloseEditor);
    };
  ...
  useEffect(() => {
    ...
    if (pending) {
      (window as any).__tl_pendingReportSelect = null;
      window.dispatchEvent(new CustomEvent('tl-select-report', { detail: pending }));
    }
    fetchReports();
  }, [active]);
  ...
  useEffect(() => {
    if (appliedReport) {
      if (appliedReport.title) setTitle(appliedReport.title);
      if (appliedReport.content) setContent(appliedReport.content);
    }
  }, [appliedReport]);
  ...
  useEffect(() => {
    reportEditedRef.current = false;
    setReportReady(false);
    const t = setTimeout(() => setReportReady(true), 1000);
    return () => clearTimeout(t);
  }, [selected?.id]);
  const onReportContent = (val: string) => {
    setContent(val);
    if (reportReady) reportEditedRef.current = true;
  };
  const onReportTitle = (e: any) => {
    setTitle(e.target.value);
    if (reportReady) reportEditedRef.current = true;
  };
  const onReportType = (e: any) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const saved = await api.report.save({
        reportType,
        title: t,
        content,
        id: selected?.id || currentReportId || undefined,
      });
      if (saved?.id) {
        setCurrentReportId(saved.id);
        if (!selected?.id) setSelected({ ...selected, id: saved.id, status: 'draft' });
      }
      reportEditedRef.current = false;
      setReportReady(false);
      ...
      alert(tt2('report.saveFailed', lang));
    }
    setSaving(false);
  };
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      onEditingReport?.(null);
    } catch {
      alert(tt2('report.deleteFailed', lang));
    }
    setDeleting(false);
    setDeleteTarget(null);
  };
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      for (const file of sendAttachments) {
        ...
        atts.push({ filename: file.name, content: b64, contentType: file.type || 'application/octet-stream' });
      }
      const body = sendBody || content || '';
      const html = await marked.parse(body, { async: true });
      const result = await api.email.sendReport({
        to: sendTo,
        cc: sendCC,
        subject,
        html: `<!DOCTYPE html><html><head><meta charset="utf-8"><style>table{border-collapse:collapse;width:100%;margin:12px 0}th,td{border:1px solid #ddd;padding:8px 12px;text-align:left}th{background:#f5f5f5;font-weight:600}</style></head><body style="font-family:sans-serif"><div style="max-width:600px;margin:0 auto;padding:20px">
${html}</div></body></html>`,
        attachments: atts,
      });
      if (!result?.ok) {
        setSendResult({ key: 'smtp-fail', errorDetail: result?.error });
        setSending(false);
        return;
      }
      setShowSendDialog(false);
      setSendResult({ key: 'success' });
      const saved = await api.report.save({
        reportType,
        title: t,
        content,
      ...
      if (saved?.id) {
        await api.report.markSent(saved.id);
        fetchReports();
        setSelected(null);
        setTitle('');
        setContent('');
  ...
  return {
    ...
    sendResult,
    setSendResult,
    sendResultMsg: sendResultText(sendResult, lang),
    sendTo,
    setSendTo,
    sendCC,
    setSendCC,
