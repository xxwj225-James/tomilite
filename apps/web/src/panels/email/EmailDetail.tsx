      ...
      >
        ...
        {isDone ? (
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--muted)' }}>
              {t('from')}
              {email?.fromAddr || '—'}
              {/* `date` is UTC — see EmailList's dateDisplay. */}
              {email?.date && (
                <span style={{ marginLeft: 8 }}>
                  {t('received')}: {formatDbShort(email.date)}
                </span>
              )}
            </div>
          </div>
        ...
        {isDone && email?.description?.includes('**已回复**:') && (
          <div style={{ background: 'color-mix(in srgb, var(--green) 8%, transparent)', borderRadius: 8, padding: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 6, color: 'var(--green)' }}>
              {t('replySent')}
            </div>
            <div style={{ fontSize: 12, whiteSpace: 'pre-wrap', lineHeight: 1.6, color: 'var(--ink)' }}>
          ...
          <div style={{ background: 'var(--surface2)', borderRadius: 8, padding: 12 }}>
            ...
            <div style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                <span
                  style={{ fontSize: 11, fontWeight: 600, color: 'var(--muted)', minWidth: 36, lineHeight: '26px' }}
                >
                  {t('to')}
                </span>
                <input
                  value={(get('sendTo') as string) || ''}
                  onChange={(e) => get('setSendTo')(e.target.value)}
                  style={{
                    flex: 1,
                    fontSize: 12,
                    padding: '4px 8px',
              ...
              <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                ...
                <input
                  value={(get('sendCC') as string) || ''}
                  onChange={(e) => get('setSendCC')(e.target.value)}
                  style={{
                    flex: 1,
                    fontSize: 12,
                    padding: '4px 8px',
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <input
                  value={(get('sendSubject') as string) || ''}
                  onChange={(e) => get('setSendSubject')(e.target.value)}
                  style={{
                    flex: 1,
                    fontSize: 12,
                    padding: '4px 8px',
        ...
        {get('emailFullBody') && (
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 6, color: 'var(--ink)', flexShrink: 0 }}>
              {t('original')}
            </div>
            <div
              style={{ fontSize: 12, lineHeight: 1.6, flex: 1 }}
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(get('emailFullBody') as string) }}
            />
          </div>
        )}
        ...
        {!isDone && (
          <div style={{ display: 'flex', gap: 8, marginTop: 4, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
            ...
            {(email?.category === 1 || email?.category === 2) &&
              !get('draftGenerating') &&
              !get('replyText') &&
              !get('isReplying') && (
            ...
            {(get('replyText') || get('isReplying')) && (
              <button
                ...
                onClick={() => get('handleSendReply')()}
                disabled={get('sending') as boolean}
              >
                {get('sending') ? (
                  t('sending')
                ) : (
            ...
            <button
              ...
              onClick={() => (get('emailFullBody') ? get('setEmailFullBody')(null) : get('handleReadFullEmail')())}
              disabled={get('emailLoading') as boolean}
            >
              {get('emailLoading') ? (
                t('loading')
              ) : (
            ...
            {get('linkedIssue') ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 10, color: 'var(--muted)' }}>{_t('emailPanel.trackedAs')}:</span>
                <a
                  onClick={(e) => {
                    e.preventDefault();
                    get('openLinkedTask')();
              ...
              <button
                className="btn btn-sm"
                style={{ background: 'var(--surface2)', color: 'var(--purple)', border: '1px solid var(--purple)' }}
                onClick={() => get('handleLinkTask')()}
                disabled={get('linkingTask') as boolean}
              >
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                  {get('linkingTask') ? (
            ...
            <button
              className="btn btn-sm"
              style={{ background: 'var(--surface2)', color: 'var(--amber)', border: '1px solid var(--amber)' }}
