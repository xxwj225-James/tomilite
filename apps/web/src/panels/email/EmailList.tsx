...
// Category accents. They map onto the semantic tokens instead of carrying their
// own palette, and each background derives from its own accent — the literals
// that used to sit here (`#fef0ef`, `#fef9e7`, …) were light-mode tints that
// became near-white blocks under dark mode's light `--ink`. The rest of this
// file already derived its fills with color-mix; this table was the last
// holdout.
const CAT_META: Record<number, { icon: string; label: string; color: string; bg: string }> = {
  1: { icon: '🔴', label: 'Urgent', color: 'var(--red)', bg: 'color-mix(in srgb, var(--red) 12%, var(--surface))' },
  2: {
    icon: '🟡',
  ...
  3: {
    ...
    label: 'Notifications',
    color: 'var(--purple)',
    bg: 'color-mix(in srgb, var(--purple) 12%, var(--surface))',
  },
  4: { icon: '🔵', label: 'Other', color: 'var(--muted)', bg: 'color-mix(in srgb, var(--muted) 12%, var(--surface))' },
};
...
function TreeLines({ lines }: { lines: Array<{ x1: number; y1: number; x2: number; y2: number; color: string }> }) {
  ...
  return (
    ...
    >
      {lines.map((l, i) => (
        // `stroke` as an attribute would not resolve the var() these colours now
        // are — SVG presentation attributes are not CSS declarations.
        <line
          key={i}
          x1={l.x1}
          y1={l.y1}
...
export function EmailList(p: Record<string, unknown>) {
  ...
  useEffect(() => {
    ...
    [1, 2, 3, 4].forEach((cat) => {
      const color = CAT_META[cat]?.color || 'var(--muted)';
      ...
      if (topicCards.length > 0) {
        ...
        lines.push({
          x1: firstTC.left + firstTC.width / 2 - rect.left,
          y1: branchY,
          x2: lastTC.left + lastTC.width / 2 - rect.left,
          y2: branchY,
          color,
        });
        ...
        topicCards.forEach((tc) => {
          ...
          emailItems.forEach((ei) => {
            ...
            lines.push({
              x1: tx,
              y1: tcr.bottom - rect.top,
              x2: er.left + er.width / 2 - rect.left,
              y2: er.top - rect.top,
              color,
            });
          });
  ...
  const filterEmails = (emails: EmailItem[]) => {
    ...
    return emails.filter(
      (e) => (e.subject || '').toLowerCase().includes(s) || (e.fromAddr || '').toLowerCase().includes(s),
    );
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      >
        ...
        <span style={{ fontSize: 10, color: 'var(--muted)', whiteSpace: 'nowrap' }}>
          {counts.all} {lang === 'zh' ? '封' : lang === 'ja' ? '通' : ''}
        </span>
      </div>
        ...
        {!get('configLoaded') ? (
          ...
          >
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--ink)', marginBottom: 6 }}>
              {tt2('emailList.noConfigTitle', lang)}
            </div>
            ...
            >
              {tt2('emailList.goToSettings', lang)}
            </button>
          </div>
        ) : (
          <div style={{ padding: '12px 16px', position: 'relative', zIndex: 1 }}>
                  ...
                  <div key={cat} data-cat-card={cat} style={{ flex: '1 1 0', minWidth: 0 }}>
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 12,
                        border: `2px solid ${isExpanded ? meta.color : 'var(--edge)'}`,
                        textAlign: 'center',
                        transition: 'all .2s',
                        boxShadow: isExpanded
                          ? `0 2px 12px color-mix(in srgb, ${meta.color} 20%, transparent)`
                          : 'none',
                      ...
                      >
                        {st.unread > 0 && (
                          <span style={{ color: 'var(--amber)', fontWeight: 600 }}>
                            {st.unread} {lang === 'zh' ? '未読' : lang === 'ja' ? '未読' : 'unread'}
                          </span>
                        )}
                        {st.needsReply > 0 && (
                          <span style={{ color: meta.color, fontWeight: 600 }}>
                            {st.needsReply} {lang === 'zh' ? '要返信' : lang === 'ja' ? '要返信' : 'reply'}
                          </span>
                        )}
                      </div>
                    </div>
            ...
            {[1, 2, 3, 4].map((cat) => {
              ...
              const catDescKeys: Record<number, string> = {
                1: 'emailList.cat1Desc',
                2: 'emailList.cat2Desc',
                3: 'emailList.cat3Desc',
                4: 'emailList.cat4Desc',
              };
                ...
                <div key={`area-${cat}`} data-cat-area={cat} style={{ marginTop: 8 }}>
                  ...
                  >
                    <span style={{ fontWeight: 600, color: meta.color }}>
                      {CAT_LABEL_CLEAN[cat]?.[lang] || CAT_LABEL_CLEAN[cat]?.en}
                    </span>
                  ...
                  {(() => {
                    ...
                    const llmGroups = (
                      get('subGroups') as Record<number, Array<{ groupKey: string; label: string; emailIds: string[] }>>
                    )?.[cat];
                    ...
                    if (groups.length === 0)
                      return <EmailTable emails={all} color={meta.color} get={get} lang={lang} />;
                    return (
                      <>
                        <div
                          style={{
                              ...
                              <div
                                ...
                                style={{
                                  ...
                                  background: isOpen
                                    ...
                                    : `color-mix(in srgb, ${meta.color} 5%, var(--surface2))`,
                                  borderRadius: 10,
                                  border: `2px solid ${isOpen ? meta.color : 'var(--edge)'}`,
                                  minWidth: 110,
                                  textAlign: 'center',
                                  transition: 'all .15s',
                                  boxShadow: isOpen
                                    ? `0 2px 8px color-mix(in srgb, ${meta.color} 20%, transparent)`
                                    : 'none',
                                }}
                              >
                                ...
                                <div style={{ fontSize: 9, color: 'var(--muted)', marginTop: 1 }}>
                                  {g.emails.length} {lang === 'zh' ? '封' : lang === 'ja' ? '通' : 'emails'}
                                </div>
                              </div>
                            );
                          })}
      ...
      {selectedIds.size > 0 && (
        ...
        >
          ...
          <button
            ...
            onClick={() => get('batchDismiss')()}
            disabled={get('batchDismissing') as boolean}
          >
            {get('batchDismissing') ? tt2('emailDetail.batchDismissing', lang) : tt2('emailDetail.batchDismiss', lang)}
          </button>
          <button
            className="btn-ghost btn-xs"
            style={{ color: 'var(--muted)', fontSize: 10, opacity: get('batchDismissing') ? 0.4 : 1 }}
            disabled={!!get('batchDismissing')}
            onClick={() => get('setSelectedIds')(new Set())}
          >
          ...
          >
            <span style={{ width: 28 }} onClick={(e) => e.stopPropagation()}>
              <input
                ...
                disabled={!!get('batchDismissing')}
                onChange={() => get('toggleSelect')(email.id)}
              />
            </span>
            <span
              onClick={() => get('selectEmail')(email)}
              title={email.fromAddr || ''}
              style={{
                width: '32%',
                overflow: 'hidden',
            ...
            <span
              onClick={() => get('selectEmail')(email)}
              style={{ width: '18%', color: 'var(--muted)', fontSize: 12, whiteSpace: 'nowrap', cursor: 'pointer' }}
            >
