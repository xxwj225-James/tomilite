...
export function useChatSessions() {
  ...
  const setMessages = useCallback(
    (messagesOrFn: any[] | ((prev: any[]) => any[]), sessionId?: string) => {
  ...
  const getMessagesSetter = useCallback(
    (sessionId: string) => {
  ...
  // Auto-titling used to live here and wrote to `SessionData.title`. That field
  // is rendered by nothing — the sidebar reads its own list from the session
  // manager — so a fix here could never reach the screen. It is in
  // useSessionManager now, next to the list it has to change.
