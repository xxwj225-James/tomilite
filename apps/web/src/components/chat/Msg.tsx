...
export function Msg({
  ...
  useEffect(() => {
    if (thinkingOpen && thinkingRef.current) thinkingRef.current.scrollTop = thinkingRef.current.scrollHeight;
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'msg-bubble',
          role === 'user' ? 'msg-bubble--user' : 'msg-bubble--assistant',
          role === 'assistant' && 'md-preview',
        )}
        style={
          isMonitor
            ? {
      ...
      >
        {isApiKeyHint ? (
          ...
          <span
            dangerouslySetInnerHTML={{
              __html:
                role === 'assistant'
                  ? sanitizeHtml(marked.parse(safeText) as string)
                  : sanitizeHtml(safeText.replace(/\n/g, '<br>')),
            }}
          />
        )}
          ...
          >
              ...
              {_t(
                card.type === 'note'
                  ...
                  : card.type === 'report'
                    ...
                    : 'app.entityTask',
                lang,
              ) || (card.type === 'note' ? 'note' : card.type === 'report' ? 'report' : 'task')}
              {card.resolved ? tr(lang, ' · 已处理', ' · 処理済み', ' · Resolved') : ''}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              <button
                ...
                disabled={!!card.resolved || thinking}
                style={{
                  background: card.resolved ? 'var(--surface2)' : 'var(--amber)',
                  color: card.resolved ? 'var(--muted)' : 'var(--ink)',
                  fontWeight: 700,
                  padding: '5px 14px',
                  opacity: thinking ? 0.5 : 1,
                  cursor: card.resolved ? 'not-allowed' : 'pointer',
                }}
                onClick={() => {
                  if (!thinking && !card.resolved)
                    window.dispatchEvent(new CustomEvent('tl-force-create', { detail: card }));
                }}
              >
                <svg
                  width="12"
                ...
                {tr(lang, '强行创建', '強制作成', 'Force Create')}
              </button>
              <button
                className="btn btn-xs"
                disabled={!!card.resolved || thinking}
                ...
                onClick={() => {
                  if (!thinking && !card.resolved)
                    window.dispatchEvent(new CustomEvent('tl-cancel-dedup', { detail: card }));
                }}
              >
          ...
          card && (
            ...
            >
              ...
              <div
                style={{
                  fontWeight: 600,
                  marginBottom: card.description ? 2 : 6,
                  lineHeight: 1.3,
                  color: 'var(--ink)',
                }}
              >
              ...
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                {card.type === 'task' && (
                  <button
                    className="btn btn-brand btn-xs"
                    onClick={() => {
                      window.dispatchEvent(new CustomEvent('tl-open-card', { detail: card }));
                    }}
                  >
                    {tr(lang, '👁 查看', '👁 表示', '👁 View')}
                  </button>
                ...
                {card.type === 'task' && (
                  <button
                    ...
                    onClick={() => {
                      window.dispatchEvent(new CustomEvent('tl-edit-card', { detail: card }));
                    }}
                  >
                    {tr(lang, '✏️ 编辑', '✏️ 編集', '✏️ Edit')}
                  </button>
                )}
                {card.type === 'note' && (
                  <button
                ...
                <button
                  ...
                  onClick={() => {
                    window.dispatchEvent(new CustomEvent('tl-delete-card', { detail: card }));
                  }}
                >
                    ...
                    onClick={async () => {
                      ...
                      if (!api?.pickSaveFile) {
                        window.dispatchEvent(
                          new CustomEvent('tl-save-result', {
                            detail: {
                              ok: false,
                              message: tr(
                                lang,
                      ...
                      try {
                        if (card.type === 'export_pdf') {
                            ...
                            window.dispatchEvent(
                              new CustomEvent('tl-save-result', {
                                detail: {
                                  ...
                                  message: tr(
                                    lang,
                                    'PDF 导出不可用（非 Electron 环境）',
                                    'PDFエクスポートは利用できません（Electron環境以外）',
                                    'PDF export unavailable (non-Electron)',
                                  ),
                                },
                              }),
                            );
                          ...
                          // 并行：后台开始渲染 PDF 的同时弹出保存对话框 —— 对话框立即出现，
                          // 用户选路径的几秒正好覆盖 printToPDF 耗时，不再干等。
                          const printPromise = api
                            .printPdf(card.html ?? '', card.title)
                            .catch((e: any) => ({ ok: false, error: (e && e.message) || String(e) }));
                          const defaultName = card.title.replace(/[<>:"/\\|?*]/g, '_').replace(/\.pdf$/i, '') + '.pdf';
                          const pdfPath = await api.pickSaveFile(defaultName, [{ name: 'PDF', extensions: ['pdf'] }]);
                          if (!pdfPath) return;
                            ...
                            window.dispatchEvent(
                              new CustomEvent('tl-save-result', {
                                detail: {
                                  ...
                                  message:
                                    printRes?.error ||
                                    tr(lang, 'PDF 生成失败', 'PDF生成に失敗しました', 'PDF generation failed'),
                                },
                              }),
                            );
                            return;
                          }
                          api.copyFile(pdfPath, printRes.filePath);
                          window.dispatchEvent(
                            new CustomEvent('tl-save-result', {
                              detail: {
                                ok: true,
            ...
            {staged.original && staged.title !== staged.original.title && staged.title && (
              <div style={{ fontSize: 10, marginBottom: 2, lineHeight: 1.6, color: 'var(--ink)' }}>
                ...
                <span style={{ textDecoration: 'line-through', color: 'var(--brand)', opacity: 0.7 }}>
                  {staged.original.title?.substring(0, 60)}
                </span>
            ...
            {staged.content && (
              <div style={{ marginBottom: 4 }}>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: sanitizeHtml(
                      marked.parse(((staged as any)._full?.content || staged.content || '') as string) as string,
                    ),
                  }}
                />
              </div>
            ...
            {staged.original && staged.description !== staged.original.description && staged.description && (
              <div style={{ fontSize: 10, marginBottom: 2, color: 'var(--muted)' }}>
                {tr(
                  ...
                  `説明変更済み (${staged.description?.length || 0} 文字)`,
                  `Modified (${staged.description?.length || 0} chars)`,
                )}
              </div>
            )}
            <div style={{ display: 'flex', gap: 6 }}>
              {onApply && (
                <button className="btn btn-brand btn-xs" onClick={() => onApply(staged)}>
                  {t('btn.apply', lang)}
                </button>
              )}
              {staged.original && onUndo && (
                <button className="btn btn-secondary btn-xs" onClick={() => onUndo(staged)}>
                  {t('btn.undo', lang)}
                </button>
              )}
            </div>
        ...
        {pinnable && onPin && (
          ...
          >
            {isPinned
              ? tr(lang, '📌 已置顶', '📌 ピン留め中', '📌 Pinned')
              : tr(lang, '📌 置顶', '📌 ピン留め', '📌 Pin to top')}
          </button>
        )}
      </div>
    </div>
