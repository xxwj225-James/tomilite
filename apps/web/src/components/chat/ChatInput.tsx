...
export function ChatInput({ query, setQuery, attachedFiles, onRemoveFile, onFiles, thinking, onSend, onStop, textareaRef }: {
  query: string;
  ...
  return (
    <div className="chat-input-row">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, paddingBottom: 6 }}>
          {attachedFiles.map((f, i) => (
            <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'var(--surface2)', border: '1px solid var(--edge)', borderRadius: 14, padding: '3px 10px', fontSize: 11, color: 'var(--ink)' }}>
              📎 {f.name}
              <button onClick={() => onRemoveFile(i)} style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: 13, padding: 0, lineHeight: 1 }}>×</button>
            </span>
          ))}
        </div>
      )}
      <div className="chat-input-inner">
        ...
        <input id="file-upload" type="file" multiple hidden onChange={async (e) => { if (e.target.files) { await onFiles(e.target.files); e.target.value = ''; } }} />
        <div className="chat-input-wrap">
          <textarea ref={textareaRef} className="chat-textarea" placeholder={t('chat.placeholder', lang)} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); if (query.trim()) { onSend(); } else { onSend(); } } if (e.key === 'Escape' && thinking) { e.preventDefault(); onStop(); } }} rows={1} />
          {thinking ? (
            <button className="chat-send-btn chat-send-btn--stop" onClick={onStop} title={t('chat.sendToInterrupt', lang)} style={{ background: 'var(--brand)', border: 'none' }}><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" stroke="none"><rect x="4" y="4" width="16" height="16" rx="2"/></svg></button>
          ) : (
            <button className={cn('chat-send-btn', query.trim() ? 'chat-send-btn--active' : 'chat-send-btn--idle')} onClick={onSend} disabled={!query.trim()} title={t('chat.sendEnter', lang)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 10 4 15 9 20" /><path d="M20 4v7a4 4 0 01-4 4H4" /></svg></button>
          )}
        </div>
      </div>
