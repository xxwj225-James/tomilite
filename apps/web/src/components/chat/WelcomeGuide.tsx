...
export function WelcomeGuide({
  ...
  // Read straight from the store rather than through App: the theme is not the
  // guide's state, and a pass-through prop would keep App holding a value it
  ...
  return (
    <div className="welcome">
      ...
      <div className="welcome-title">{t('app.welcomeTitle', lang)}</div>
      <div className="welcome-desc">{t('app.welcomeDesc', lang)}</div>
      ...
      <div style={{ marginTop: 16, maxWidth: 380, textAlign: 'left', margin: '16px auto 0' }}>
        ...
        >
          ...
          <span style={{ flex: 1, fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--ink)' }}>
            {t('app.welcomeSetupLlm', lang)}
          </span>
          {llmConfigured ? (
            <span style={{ fontSize: 'var(--text-xs)', color: 'var(--green)', fontWeight: 600 }}>✅</span>
          ) : (
            <button
              className="btn btn-brand btn-xs"
              style={{ whiteSpace: 'nowrap' }}
              onClick={() => onConfigure('llm')}
            >
              {t('app.welcomeSetupConfigure', lang)}
            </button>
          )}
        </div>
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted)', margin: '0 8px 10px', lineHeight: 1.5 }}>
        ...
        >
          <span style={{ fontSize: 17 }}>📅</span>
          <span style={{ flex: 1, fontSize: 'var(--text-xs)', fontWeight: 600, color: 'var(--ink)' }}>
            {t('app.welcomeSetupStandup', lang)}
          </span>
          {standupConfigured ? (
            <span style={{ fontSize: 'var(--text-xs)', color: 'var(--green)', fontWeight: 600 }}>✅</span>
          ) : (
        ...
        >
          ...
          {mcpConfigured ? (
            ...
            <button
              ...
              onClick={() => onConfigure('mcpServers')}
            >
              {t('app.welcomeSetupConfigure', lang)}
            </button>
          )}
            ...
            {THEMES.map((th) => (
              <button
                ...
                style={{
                  ...
                  height: 18,
                  borderRadius: 4,
                  background: THEME_COLORS[th],
                  border: theme === th ? '2px solid var(--ink)' : '1px solid var(--edge)',
                  cursor: 'pointer',
                }}
              />
            ))}
        ...
        {llmConfigured && emailConfigured && gitConfigured && apikeyConfigured && standupConfigured && mcpConfigured ? (
          <div style={{ textAlign: 'center', padding: '12px 0 4px' }}>
            ...
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--green)', fontWeight: 600, marginBottom: 8 }}>
      ...
      <div className="suggestions" style={{ marginTop: 10 }}>
        {['app.sugg1', 'app.sugg2', 'app.sugg3', 'app.sugg4', 'app.sugg5'].map((k) => (
          <span key={k} className="suggestion-chip" onClick={() => onSuggestion(t(k as I18NKey, lang))}>
            {t(k as I18NKey, lang)}
          </span>
        ))}
      </div>
