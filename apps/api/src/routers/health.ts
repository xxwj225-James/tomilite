...
export const healthRouter = router({
  personalHealth: publicProcedure.input(z.object({ lang: z.string().default('en'), force: z.boolean().default(false) })).query(async ({ input }) => {
    ...
    if (!input.force) {
      ...
      if (cached) {
        return {
          score: cached.healthScore, level: cached.healthLevel,
          dimensions: JSON.parse(cached.dimensions || '{}'),
          summary: cached.summary, recommendations: [],
          gitToday: 0, total: cached.total, done: 0, inProgress: 0, todo: 0,
          cached: true, generatedAt: cached.createdAt,
        };
      }
    }
    ...
    const recentlyDone = allIssues.filter(i => i.status === 'done' && i.updatedAt && (now - new Date(i.updatedAt).getTime()) < weekMs);
    const stale = allIssues.filter(i => i.status !== 'done' && i.createdAt && (now - new Date(i.createdAt).getTime()) > 14 * dayMs);
    ...
    // Try LLM polish
    const fallbackSummary = t('health.fallbackSummary', input.lang, { score: String(overall), level, done: String(done), inProgress: String(inProgress), todo: String(todo) });
    ...
    try {
      ...
      if (provider?.apiKey) {
        ...
        const body: any = {
          model: cfg?.flashModel || cfg?.proModel , max_tokens: 500,
          messages: [{ role: 'user', content: `You are a thoughtful developer coach. Write a detailed health analysis (3-4 paragraphs, 150-200 words total) based on these metrics:
...
Be warm, encouraging, and specific. Reference the actual numbers. Write in ${langLabel}. Do NOT add any greeting, preamble, or closing — start directly with the analysis.` }],
        ...
        const resp = await fetch(`${baseUrl}/chat/completions`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${await decrypt(provider.apiKey)}` },
        ...
        if (resp.ok) {
          ...
          if (content) {
            ...
            content = content.replace(/^(好的[，,]?|OK[，,]?\s*|Based on the (data|metrics)[，,]?\s*|Here'?s?\s*(is|are)\s*(my|the)\s*(analysis|assessment)[，,:]?\s*)/i, '').trim();
            summary = content;
          }
        }
      }
    ...
    for (const s of raw) {
      ...
      if (!seen.has(day)) { seen.add(day); dailyScores.unshift(s.healthScore); }
      if (dailyScores.length >= 6) break;
    }
    const history = [...dailyScores, overall];
    const prevAvg = dailyScores.length > 0
      ? dailyScores.reduce((s, v) => s + v, 0) / dailyScores.length
      : overall;
    ...
    await prisma.userHealthSnapshot.create({
      data: {
        healthScore: overall, healthLevel: level,
        dimensions: JSON.stringify(dimensions),
        summary, total, lang: input.lang, createdAt: now2,
      },
    });

    return { score: overall, level, dimensions, summary, recommendations, gitToday: gitCommits.length, total, done, inProgress, todo, generatedAt: now2, trend: { direction, history, prevAvg: Math.round(prevAvg) } };
  }),

  healthHistory: publicProcedure
  ...
  taskStats: publicProcedure.query(async () => {
    ...
    if (total === 0) return { total: 0, byStatus: {}, byPriority: {}, byType: {}, completionRate: 0, done: 0, recentlyDone: 0 };
    ...
    for (const i of issues) { byPriority[i.priority] = (byPriority[i.priority] || 0) + 1; }

    const TYPES = ['task', 'bug', 'story'];
    const byType: Record<string, number> = {};
    for (const t of TYPES) byType[t] = 0;
    for (const i of issues) { const t = TYPES.includes(i.type) ? i.type : 'task'; byType[t] = (byType[t] || 0) + 1; }

    const done = byStatus['done'] || 0;
    const weekAgo = new Date(Date.now() - 7 * 86400000).toISOString();
    const recentlyDone = issues.filter(i => i.status === 'done' && i.updatedAt && i.updatedAt >= weekAgo).length;

    return { total, byStatus, byPriority, byType, completionRate: Math.round((done / total) * 100), done, recentlyDone };
  }),
});
