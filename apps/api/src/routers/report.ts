...
export const reportRouter = router({
  ...
  list: publicProcedure.input(z.object({ limit: z.number().default(10) })).query(async ({ input }) => {
    return prisma.report.findMany({ where: { archived: false }, orderBy: { createdAt: 'desc' }, take: input.limit });
  }),

  getLatest: publicProcedure.input(z.object({ reportType: z.string().optional() })).query(async ({ input }) => {
  ...
  save: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const existing = await prisma.report.findFirst({
        where: { reportType: input.reportType, title: input.title, status: 'draft' },
      });
      if (existing) {
        return prisma.report.update({
  ...
  markSent: publicProcedure.input(z.object({ id: z.string() })).mutation(async ({ input }) => {
    return prisma.report.update({ where: { id: input.id }, data: { status: 'sent' } });
  }),

  exportExcel: publicProcedure.input(z.object({ reportId: z.string() })).query(async ({ input }) => {
  ...
  exportWord: publicProcedure.input(z.object({ reportId: z.string() })).query(async ({ input }) => {
    const result = await exportToDoc(input);
    return result.error ? result : { ok: true, filePath: result.filePath, filename: result.filename };
  }),

  exportHtml: publicProcedure.input(z.object({ reportId: z.string() })).query(async ({ input }) => {
