...
export const knowledgeRouter = router({
  generate: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const hash = Buffer.from(taskList + noteList + reportList)
        .toString('base64')
        .substring(0, 32);
      ...
      try {
        ...
        if (!llm) {
          genError = t('knowledge.noLlm', input.lang);
        } else {
          if (!llm.flashModel && !llm.proModel) {
            genError = t('knowledge.noModel', input.lang);
          } else {
...
${reportList}`;
            ...
            const body: any = { model, max_tokens: 1200, messages: [{ role: 'user', content: prompt }] };
            ...
            if (!resp.ok) {
              ...
              if (content) {
                ...
                genError = `LLM returned empty response. model=${model}, finish_reason=${d.choices?.[0]?.finish_reason || 'none'}`;
              }
            }
          }
        }
