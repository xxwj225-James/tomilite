...
async function generateEveningContent(
  allIssues: any[],
  allNotes: any[],
  allReports: any[],
  gitCommits: any[],
  ...
  const commitList =
    gitCommits
      .slice(0, 15)
      .map((c) => `[${c.hash?.substring(0, 7)}] ${c.message?.substring(0, 80)}`)
      .join('\n') || 'none';
  const moveList = moves.map((m) => `${m.key} ${m.title}: ${m.from}→${m.to}`).join(', ') || 'none';

  // Try LLM (BYOK or hosted gateway)
  try {
    ...
    if (llm) {
      ...
      const emailList =
        todayEmails
          .map(
            (e) =>
              `${e.category === 1 ? '🔴' : e.category === 2 ? '🟡' : '🔵'} [${e.fromAddr?.substring(0, 20)}] ${e.subject?.substring(0, 60)}`,
          )
          .join('\n') || 'none';
...
- Status changes: ${moveList}`;
  ...
  const PRIO_KEY: Record<string, string> = {
    critical: 'standup.criticalHeader',
    high: 'standup.highHeader',
    medium: 'standup.mediumHeader',
    low: 'standup.lowHeader',
  };
  const STATUS_KEY: Record<string, string> = {
    done: 'standup.statusDone',
    in_progress: 'standup.statusProgress',
    todo: 'standup.statusTodo',
  };
...
export async function checkAndGenerateEvening(lang: string = 'en'): Promise<string | null> {
  try {
    ...
    console.error('[Standup] Evening check failed:', e);
    return null;
  }
}
    ...
    try {
      ...
      if (llm) {
        ...
        const overdueList =
          overdue
            .slice(0, 5)
            .map((o) => `${o.key} ${o.title} (${o.daysStale}d)`)
            .join('\n') || 'none';
...
Style: Warm and encouraging. ONLY use real tasks from the list above. Skip empty priorities. Keep under 250 words.`;
        ...
        if (resp.ok) {
          const d = await resp.json();
          const ai = (d.choices?.[0]?.message?.content || '').trim().replace(/☀️?/g, '🌅');
          if (ai) greeting = ai;
        }
      }
    } catch {
  ...
  getEveningReport: publicProcedure.input(z.object({ lang: z.string().default('en') })).mutation(async ({ input }) => {
    const today = new Date().toISOString().substring(0, 10);
    const langLabel =
      input.lang === 'zh' ? '📋 晚报' : input.lang === 'ja' ? '📋 イブニングレポート' : '📋 Evening Report';
    ...
    const content = await generateEveningContent(
      allIssues,
      allNotes,
      allReports,
      gitCommits,
    ...
    try {
      ...
      console.error('[Standup] Failed to save evening report:', e?.message || e);
    }
  ...
  getSettings: publicProcedure.query(async () => {
    try {
      ...
      return { morning: true, morningTime: '09:00', evening: false, eveningTime: '18:00' };
    }
  }),
