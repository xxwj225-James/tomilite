  ...
  reviewIssue: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (keywords.length > 0) {
        ...
        const allMatches = await prisma.issue.findMany({
          where: { projectId: 'proj-default', OR: orClauses },
          take: 10,
        });
        similarIssues = allMatches
          .filter((i) => {
          ...
          .map((i) => ({
            ...
            matchScore: Math.min(
              100,
              keywords.filter((k) => (i.title + (i.description || '')).toLowerCase().includes(k)).length * 25,
            ),
          }));
      }
      if (similarIssues.length > 0) {
        ...
        if (highestMatch.matchScore >= 70) {
          duplicateRisk = `high — ${highestMatch.key}: ${highestMatch.title}`;
          notes.push({
            level: 'danger',
            msg: `Possible duplicate of ${highestMatch.key} (${highestMatch.matchScore}% match).`,
          });
          score -= 30;
        } else {
          notes.push({ level: 'warning', msg: `Similar issue found: ${highestMatch.key}: ${highestMatch.title}` });
        }
      }
      if (!title || title.length < 10) {
        notes.push({ level: 'warning', msg: 'Title is too short.' });
        suggestions.push('Add more detail to the title');
        score -= 10;
      } else {
        score += 15;
      }
      if (!description || description.length < 20) {
        notes.push({ level: 'warning', msg: 'No description.' });
        suggestions.push('Add a description with steps to reproduce.');
        score -= 10;
      } else {
        score += 10;
      }
      if (storyPoints && storyPoints > 13) {
        notes.push({ level: 'warning', msg: `Large story (${storyPoints}sp). Consider splitting.` });
        suggestions.push('Break this into 2-3 smaller issues.');
      }
      if (!storyPoints) {
        suggestions.push('Add story points for sprint planning visibility.');
      }
          ...
          const resp = await fetch(`${llm.baseUrl}/chat/completions`, {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: 'user',
                  content: `Review this issue briefly (1-2 sentences): title="${title}", type=${type}, priority=${priority}. Give one actionable improvement suggestion.`,
                },
              ],
            }),
            signal: AbortSignal.timeout(15000),
