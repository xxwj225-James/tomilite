...
export async function classifyGuard(
  ...
  const cleanMsg = message.replace(/\[(Note editor|Task editor|New task|Report editor)[^\]]*\]/g, '').replace(/\(unsaved\)/g, '').trim();

  // Phase 1: Keyword pre-filter
  const isExplicitCreate = /^(创建|新建|create|new\s+(task|bug|issue|feature|story|note)|add\s+(task|bug)|write\s+(a|note)|帮.*(创建|写|加))/i.test(cleanMsg) || /创建|新建|create_/.test(cleanMsg.substring(0, 30));
  const wantsCreateNote = /(总结|写|做|记|新建|创建|建|加|生成|弄).{0,4}(笔记|note)/i.test(cleanMsg) || /(笔记|note).{0,4}(总结|写|做|记)/i.test(cleanMsg);
  const wantsCreateTask = /(加|添|新建|创建|建|生成|弄|派).{0,3}(任务|task|bug|issue)/i.test(cleanMsg) || /(任务|task).{0,3}(加|添|新建|创建|建)/i.test(cleanMsg);

  if (!isExplicitCreate && !wantsCreateNote && !wantsCreateTask) {
    needsWebSearch = true;
    intentHint = '\n[INTERNAL ROUTING (do NOT mention this to user): The user is chatting or asking questions. For factual questions, recent events, or anything requiring current information — you MUST call web_search FIRST, then answer based on the results. Do NOT just say you will search — actually CALL the tool. Prefer using tools over plain text replies for practical requests.]';
  }
  ...
  if (!intentHint) try {
    ...
    const guardResp = await fetch(`${config.baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({ model: config.flashModel, messages: [{ role: 'user', content: guardPrompt }], max_tokens: 200, temperature: 0, response_format: { type: 'json_object' },
        ...(config.baseUrl.includes('moonshot') || config.baseUrl.includes('deepseek') ? { thinking: { type: 'disabled' } } : {}),
