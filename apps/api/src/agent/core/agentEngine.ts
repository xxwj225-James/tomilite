...
/**
 * Check for existing duplicates of a create_* request.
 * Pushes the tool response for tcId immediately (DeepSeek requires it), but returns
 * the guidance text for the CALLER to defer — pushing a user message here would
 * interleave it between parallel tool_calls and their tool responses (DeepSeek 400).
 */
async function checkDedup(
  ...
  const where: any =
    toolName === 'create_report' ? { title: args.title } : { projectId: DEFAULT_PROJECT_ID, title: args.title };
  ...
  messages.push({
    ...
    content: JSON.stringify({
      blocked: true,
      duplicates: dups.map((d: any) => ({ key: target.resultKey(d), title: d.title })),
    }),
  });
  return `⚠️ ${label} already exists (${dupKeys}). Do NOT try to create it again. Reply to the user.`;
}
...
export async function runAgentLoop(
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    // Guidance user-messages produced while executing tools are deferred here and flushed
    // AFTER all tool responses for this round — DeepSeek rejects user messages interleaved
    // between an assistant's parallel tool_calls and their tool responses (HTTP 400).
        ...
        const tBody = {
          ...
          messages: [
            ...messages,
            {
              role: 'user',
              content:
                'Think through this naturally in <thinking> tags, like thinking to yourself. What does the user need? How should you help? Stop at </thinking>.',
            },
          ],
          max_tokens: 1024,
          stop: ['</thinking>'],
        };
        const tResp = await fetch(config.baseUrl + '/chat/completions', {
          method: 'POST',
        ...
        if (tResp.ok) {
          ...
          if (tReader) {
            ...
            if (
              ...
              messages.push({ role: 'assistant', content: '<thinking>' + thinking + '</thinking>' });
              messages.push({ role: 'user', content: 'Now act. Call the right tool or write your answer.' });
            }
          }
        }
      } catch {
    ...
    if (result.toolCalls.length === 0) {
      ...
      const expectedTool =
        unsavedNote || noteEditorOpen
          ? 'suggest_note_edit'
          : taskEditorOpen || newTaskFormOpen
            ? 'suggest_issue_edit'
            : null;
      // #1: editor form open but suggest_*_edit not called
      if (!llmResponded && expectedTool && !calledTools.has(expectedTool)) {
        const userAskedToEdit =
          messages[0]?.content?.includes('编辑') ||
          messages[0]?.content?.includes('修改') ||
          messages[0]?.content?.includes('更新') ||
          messages[0]?.content?.includes('edit') ||
          messages[0]?.content?.includes('update') ||
          messages[0]?.content?.includes('change');
        if (userAskedToEdit || unsavedNote) {
          llmResponded = true;
          sendToken(send, 'Correcting...');
          messages.push({
            role: 'user',
            content: `⚠️ SYSTEM CHECK: You did NOT call ${expectedTool}. The user wants to modify the open form. Call ${expectedTool} with the complete content.`,
          });
          continue;
        }
      }
      ...
      const parsedGuard = (() => {
        try {
          return JSON.parse(lastGuardRaw || '{}').intent;
        } catch {
          return '';
        }
      })();
      if (!llmResponded) {
        if (
          parsedGuard === 'create_task' &&
          !calledTools.has('create_issue') &&
          !calledTools.has('force_create_issue')
        ) {
          llmResponded = true;
          ...
          messages.push({
            role: 'user',
            content:
              '⚠️ SYSTEM CHECK: The user asked to create a task. You MUST call create_issue tool with the title and details. Do NOT reply with text — call the tool first, then reply.',
          });
          continue;
        }
        if (parsedGuard === 'create_note' && !calledTools.has('create_note') && !calledTools.has('force_create_note')) {
          ...
          messages.push({
            role: 'user',
            content:
              '⚠️ SYSTEM CHECK: The user asked to create a note. You MUST call create_note tool with the title and content. Do NOT reply with text — call the tool first, then reply.',
          });
          continue;
        }
        if (parsedGuard === 'edit_note' && !calledTools.has('suggest_note_edit')) {
          ...
          messages.push({
            role: 'user',
            content:
              '⚠️ SYSTEM CHECK: The user wants to edit a note. You MUST call suggest_note_edit tool with the complete content. Do NOT reply with text — call the tool first, then reply.',
          });
          continue;
        }
      }
      ...
      const askedForReport =
        !isExportRequest &&
        (cleanMsg.includes('报告') ||
          cleanMsg.includes('report') ||
          cleanMsg.includes('日报') ||
          cleanMsg.includes('周报'));
      if (!llmResponded) {
        if (askedForReport) {
          llmResponded = true;
          ...
          messages.push({
            role: 'user',
            content:
              '⚠️ SYSTEM CHECK: The user asked to create a report. You MUST call create_report tool with the title and content. Do NOT reply with text — call the tool first, then reply.',
          });
          continue;
        }
        if (isExportRequest) {
      ...
      if (!llmResponded && needsWebSearch && !isQwen && !calledTools.has('web_search')) {
        ...
        messages.push({
          role: 'user',
          content:
            '⚠️ SYSTEM CHECK: This question requires current information. You MUST call web_search FIRST with a relevant query, then answer based on the results. Do NOT just talk about searching — actually CALL the tool now.',
        });
        continue;
      }
      ...
      if (!llmResponded && calledTools.size === 0) {
        ...
        if (['create_task', 'create_note', 'edit_note'].includes(parsedIntent)) {
          ...
          messages.push({
            role: 'user',
            content: `SYSTEM CHECK: Guard classified as "${parsedIntent}" but NO tool called. Call ${toolMap[parsedIntent]}. Do NOT fake a result.`,
          });
          continue;
        }
        ...
        if (/✅|📄|📋|📊/.test(result.content) && /TL-|创建|created|作成/.test(result.content)) {
          ...
          messages.push({
            role: 'user',
            content:
              'SYSTEM CHECK: Reply claims success but NO tool called. Call create_issue/create_note/create_report.',
          });
          continue;
        }
      }
    ...
    if (editorExpectedTool) {
      ...
      if (spuriousCreates.length > 0 && hasExpectedSuggest) {
        ...
        messages.push({
          role: 'user',
          content: `⚠️ SYSTEM: Ignored ${spuriousCreates.map((tc: any) => tc.name).join(', ')} — form is open, only ${editorExpectedTool} is appropriate here.`,
        });
    ...
    for (const tc of result.toolCalls) {
      ...
      try {
        ...
        if ((tc.name === 'create_issue' || tc.name === 'create_note' || tc.name === 'create_report') && args.title) {
          const guide = await checkDedup(tc.name, args, config, send, messages, tc.id);
          if (guide) {
            pendingGuides.push(guide);
            continue;
          }
        }

        // Normal execution
        const fixedName = toolLabels[tc.name]
          ? tc.name
          : Object.keys(toolLabels).find((k) => k.replace(/_/g, '') === tc.name.replace(/_/g, '')) || tc.name;
        ...
        // export_to_pdf carries full HTML for the renderer's printPdf IPC — strip it from LLM history (token blowup)
        const toolMsgContent =
          tc.name === 'export_to_pdf'
            ? JSON.stringify({ ...(r as Record<string, any>), html: undefined })
            : JSON.stringify(r);
        messages.push({ role: 'tool', tool_call_id: tc.id, content: toolMsgContent });
        if ((r as any)?.key) createdKeys.push((r as any).key);
        else if ((r as any)?.id && (tc.name === 'create_note' || tc.name === 'force_create_note'))
        ...
        if (tc.name === 'list_reports' && Array.isArray(r) && (r as any[]).length === 0 && args.query) {
          pendingGuides.push(
            `SYSTEM: list_reports found nothing. Instead of retrying, use search_local_data to do a full-text search across all local data. Then tell the user what you found. Do NOT output tool names as text — actually CALL the tool.`,
          );
        }
        ...
        if (typeof errMsg === 'string') {
          ...
          const hints: Record<string, string> = {
            'Unknown tool':
              'Use one of the available tools. To find notes: search_notes or search_local_data. To find reports: list_reports. To find issues: list_issues or get_issue.',
            'Report not found':
              'Use list_reports to get the correct UUID (NOT the title). If list_reports returns nothing, try search_local_data for fuzzy matching. Only give up if BOTH fail.',
            ...
            'no task editor':
              'STOP using suggest_issue_edit. Use create_issue for new tasks or update_issue(issueNumber=N) for existing ones.',
            'no note editor':
              'STOP using suggest_note_edit. Use create_note for new notes or update_note for existing ones.',
            'no report editor':
              'STOP using suggest_report_edit. Use create_report for new reports — it saves directly to DB.',
            'Invalid JSON':
              'Your tool arguments contain invalid JSON. Simplify the content — use plain text without special characters, quotes, or markdown. Keep it short.',
            'Unexpected token':
              'Your tool arguments contain invalid JSON. Simplify the content — use plain text without special characters, quotes, or markdown. Keep it short.',
          };
          const hint =
            Object.entries(hints).find(([k]) => errMsg.toLowerCase().includes(k))?.[1] ||
            'Check the error and retry with corrected parameters.';
          pendingGuides.push(`⚠️ ${tc.name} failed: ${errMsg}. ${hint}`);
        }
      } catch (e: any) {
        ...
        pendingGuides.push(
          `⚠️ ${tc.name} threw an exception: ${e.message}. Retry with corrected input or use a different approach.`,
        );
      }
    }

    // Flush deferred guidance AFTER all tool responses — keeps assistant(tool_calls) → tool(...)
    // contiguous so DeepSeek's strict validation accepts the chain (see pendingGuides note above).
    for (const guide of pendingGuides) messages.push({ role: 'user', content: guide });
    ...
    if (result.toolCalls.length > 0 && !hadError && isQwenProvider(config.baseUrl)) {
      const intentDone =
        (guardIntent === 'create_task' && (calledTools.has('create_issue') || calledTools.has('force_create_issue'))) ||
