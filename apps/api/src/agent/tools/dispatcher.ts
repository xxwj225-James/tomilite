...
import { createReport, updateReport, listReports, getReport, deleteReport, suggestReportEdit, exportToExcel, exportToDoc, exportToHtml } from './reportTools.js';
import { braveSearch, webSearch, searchLocalData } from './searchTools.js';
import { listGitCommits, listWorkspaces } from './gitTools.js';
import { listEmails, editEmailReply, sendEmailReply, readEmailOriginal, dismissEmail, deleteEmail } from './emailTools.js';
...
export async function executeAgentTool(tool: string, args: Record<string, any>): Promise<any> {
  switch (tool) {
    case 'create_issue':
    case 'force_create_issue': return createIssue(args);
    case 'get_stats': return getStats();
    case 'list_issues': return listIssues(args);
    case 'get_issue': return getIssue(args);
    case 'search_notes': {
      return searchNotesSemantic(args.query?.trim() || '', 5);
    }
    case 'list_notes': return listNotes(args);
    case 'update_issue': return updateIssue(args);
    case 'force_create_report':
    case 'create_report': return createReport(args);
    case 'update_report': return updateReport(args);
    case 'force_create_note':
    case 'create_note': return createNote(args);
    case 'update_note': return updateNote(args);
    case 'suggest_note_edit': return suggestNoteEdit(args);
    case 'suggest_issue_edit': return suggestIssueEdit(args);
    case 'suggest_report_edit': return suggestReportEdit(args);
    case 'brave_search': return braveSearch(args);
    case 'web_search': return webSearch(args);
    case 'search_local_data': return searchLocalData(args);
    case 'list_git_commits': return listGitCommits(args);
    case 'list_workspaces': return listWorkspaces(args);
    case 'shell_exec': {
    ...
    case 'list_emails': return listEmails(args);
    case 'edit_email_reply': return editEmailReply(args);
    case 'send_email_reply': return sendEmailReply(args);
    case 'read_email_original': return readEmailOriginal(args);
    case 'dismiss_email': return dismissEmail(args);
    case 'delete_email': return deleteEmail(args);
    case 'list_reports': return listReports(args);
    case 'get_report': return getReport(args);
    case 'delete_report': return deleteReport(args);
    case 'polish_report':
    ...
    case 'export_to_excel': return exportToExcel(args);
    case 'export_to_doc': return exportToDoc(args);
    case 'export_to_html': return exportToHtml(args);
    default: {
      ...
      if (parsed) {
        try {
          ...
          if (server) {
            ...
            if (server.apiKey) {
              try { apiKey = await decrypt(server.apiKey); } catch { apiKey = server.apiKey; }
            }
            let headers: Record<string, string> = {};
            if (server.headers) {
              try { headers = JSON.parse(server.headers); } catch { /* not JSON */ }
            }
            const client = createMCPClient({
              name: server.name, url: server.url, apiKey, headers,
      ...
      const err = (emailResult && typeof emailResult === 'object' && 'error' in emailResult) ? (emailResult as Record<string, unknown>).error : null;
