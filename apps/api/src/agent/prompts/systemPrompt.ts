...
export function buildSystemPrompt(params: SystemPromptParams): string {
  const { preferenceHint, learnHint, intentHint, workspaceRoots, baseUrl, model, lang, unsavedNote, noteEditorOpen, taskEditorOpen, newTaskFormOpen, reportEditorOpen, notesPanelOpen, tasksPanelOpen, reportsPanelOpen } = params;
  ...
  const hasNativeThinking = baseUrl?.includes('deepseek') || baseUrl?.includes('dashscope') || (baseUrl?.includes('moonshot') && model?.includes('kimi-k3'));
