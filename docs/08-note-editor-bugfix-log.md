...
| `agent.ts` | When `[Note editor action:]` is detected, skip the guard model and route directly to `suggest_note_edit` |
...
- **Frontend double obfuscation**: removed the second obfuscation pass on frontend index-*.js from `obfuscate.js` (the vite plugin already handles it)
